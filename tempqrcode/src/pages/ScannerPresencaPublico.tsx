...
import { useTimePrincipal } from '@/hooks/useConfiguracaoEsportiva';
...
const ScannerPresencaPublico = () => {
  ...
  // Realtime connection
  const { isConnected, subscribe, emit } = useRealtime(viagemId || '', onibusId);
  
  // Configuração esportiva
  const { time, cidade, estadio, logoUrl } = useTimePrincipal();
  
  // Determinar se é jogo em casa ou fora baseado no estádio
  const isJogoEmCasa = !viagem?.nome_estadio || viagem.nome_estadio === (estadio || "Estádio Principal");
  ...
  // Subscrever a eventos de realtime
  useEffect(() => {
    if (!viagemId || !onibusId) return;

    const unsubscribe = subscribe('scanner-publico', (event) => {
      console.log('📡 Evento realtime recebido:', event);
      
      // Se o evento é desta viagem/ônibus, atualizar dados
      if (event.data.viagemId === viagemId && 
          (!event.data.onibusId || event.data.onibusId === onibusId)) {
        
        // Atualizar passageiro específico no estado local
        setPassageiros(prev => 
          prev.map(p => 
            p.viagem_passageiro_id === event.data.passageiroId
              ? { ...p, status_presenca: event.data.novoStatus as 'pendente' | 'presente' | 'ausente' }
              : p
          )
        );

        // Adicionar às confirmações recentes se foi confirmado
        if (event.data.novoStatus === 'presente') {
          const novaConfirmacao = {
            clientes: { nome: event.data.passageiroNome },
            confirmed_at: event.data.timestamp,
            confirmation_method: event.data.method || 'manual'
          };
          
          setConfirmacoes(prev => [novaConfirmacao, ...prev.slice(0, 4)]);
        }

        toast.success(`🔄 Atualização em tempo real: ${event.data.passageiroNome} - ${event.data.novoStatus}`);
      }
    });

    return unsubscribe;
  }, [viagemId, onibusId, subscribe]);
  ...
  const handleScanSuccess = async (result: any) => {
    ...
    if (result.data) {
      ...
      await emit({
        type: 'qr_code_escaneado',
        data: {
          viagemId: viagemId || '',
          onibusId: onibusId,
          passageiroId: result.data.passageiro.id || '',
          passageiroNome: result.data.passageiro.nome,
          novoStatus: 'presente',
          timestamp: new Date().toISOString(),
          method: 'qr_code_responsavel'
        }
      });
    }
  };
  ...
  const handleMarcarPresenca = async (passageiro: Passageiro) => {
    try {
      ...
      // Emitir evento de realtime
      await emit({
        type: novoStatus === 'presente' ? 'presenca_confirmada' : 'presenca_removida',
        data: {
          viagemId: viagemId || '',
          onibusId: onibusId,
            ...
            <div className="flex flex-col md:flex-row items-center justify-between">
              <div className="flex items-center gap-4 mb-4 md:mb-0">
                <div className="flex items-center gap-2">
                  {isJogoEmCasa ? (
                    // JOGO EM CASA - Time principal primeiro
                    <>
                      {(logoUrl || viagem.logo_flamengo) ? (
                        <div className="w-12 h-12 rounded-full overflow-hidden">
                          <img 
                            src={logoUrl || viagem.logo_flamengo || `https://placehold.co/150x150/e5e7eb/9ca3af?text=${time ? time.substring(0, 3).toUpperCase() : "TIME"}`} 
                            alt={time || "Time Principal"} 
                            className="w-full h-full object-cover"
                            onError={(e) => {
                              const target = e.target as HTMLImageElement;
                              target.onerror = null;
                              target.src = `https://placehold.co/150x150/e5e7eb/9ca3af?text=${time ? time.substring(0, 3).toUpperCase() : "TIME"}`;
                            }}
                          />
                        </div>
                      ) : (
                        <div className="bg-red-600 w-12 h-12 rounded-full flex items-center justify-center">
                          <span className="text-white font-bold">{time ? time.substring(0, 3).toUpperCase() : "TIME"}</span>
                        </div>
                      )}
                    ...
                    // JOGO FORA - Adversário primeiro (mandante)
                    <>
                      {viagem.logo_adversario ? (
                        <div className="w-12 h-12 rounded-full overflow-hidden">
                          <img 
                ...
                <div>
                  <h2 className="font-bold text-lg">
                    {isJogoEmCasa ? 
                      `${time || "Time Principal"} × ${viagem.adversario}` : 
                      `${viagem.adversario} × ${time || "Time Principal"}`
                    }
                  </h2>
              ...
              <div className="flex flex-col items-end">
                <Badge className="mb-1">
                  {isJogoEmCasa ? 
                    ...
                    (viagem.nome_estadio || "Estádio Visitante")
                  }
                </Badge>
                <span className="text-sm text-muted-foreground">
                  {isJogoEmCasa ? 
                    (cidade || "Cidade Principal") : 
                    (viagem.local_jogo || "Cidade do Jogo")
                  }
                </span>
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {isConnected ? 'Sincronização ativa' : 'Reconectando...'}
                  </span>
                </div>
              </div>
            </div>
