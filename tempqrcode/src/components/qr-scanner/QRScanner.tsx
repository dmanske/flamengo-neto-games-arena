...
interface QRScannerProps {
  viagemId: string;
  onibusId?: string; // Para restringir apenas passageiros do ônibus
...
export const QRScanner: React.FC<QRScannerProps> = ({
  ...
  const startScanning = async () => {
    try {
      ...
      // Solicitar acesso à câmera de forma simples
      ...
      toast.error('Erro de câmera', {
        description: '📷 Não foi possível acessar a câmera. Verifique as permissões do navegador e tente novamente.',
        duration: 5000,
      });
      setIsScanning(false);
    }
  ...
  const handleQRCodeDetected = async (qrText: string) => {
    try {
      ...
      toast.error('Erro no processamento', {
        description: '🔧 Não foi possível processar o QR code. Verifique se o código está legível e tente novamente.',
        duration: 4000,
      });
  ...
  return (
    <Card className="w-full max-w-md mx-auto">
      ...
      <CardContent className="space-y-4">
        {/* Área do vídeo com design melhorado */}
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900 to-gray-800">
          ...
          {/* Indicador de processamento melhorado */}
