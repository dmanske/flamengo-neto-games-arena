...
import { MapPin, Calendar, Users, DollarSign, ArrowLeft, Star, Clock, ShoppingCart, Package, User } from "lucide-react";
...
const Loja = () => {
  ...
  const handleAdicionarCarrinho = (viagem: Viagem) => {
    adicionarAoCarrinho({
      id: viagem.id,
  ...
  const viagensDestaque = viagens.filter(v => v.destaque_loja);
  const viagensNormais = viagens.filter(v => !v.destaque_loja);
